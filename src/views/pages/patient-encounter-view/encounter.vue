            ...
            <VCardText class="mb-2">              
              ...
              >
                ...
                <div v-if="encounterType == 'Injectables'">
                  <VRow v-for="(item, index) in encounterList" :key="index" class="d-flex align-center justify-space-between">
                    <VCol>                      
                      <VSelect
                    ...
                    <VCol>                        
                      <VTextField v-model="encounterList[index]['quantity']" label="Quantity" type="number" variant="underlined" :rules="[requiredValidator]" density="default" style="width: 10rem;"></VTextField>                  
                    </VCol>
                    <VCol class="d-flex align-center">
                      <VTextField v-model="encounterList[index]['dosage']" label="Dosage" type="number" variant="underlined" density="default" :rules="[requiredValidator]" style="width: 5rem;"></VTextField>                  
                      <VSelect v-model="encounterList[index]['unit']" label="Unit" :items="units" :rules="[requiredValidator]" density="compact" variant="underlined" class="mt-3 ms-2" style="width: 1.5rem;" />                      
                    </VCol>
                    <VCol>
                      <VCheckbox v-model="encounterList[index]['is_add_on']" label="Add On" class="pt-4"></VCheckbox>                      
                    </VCol>
                  </VRow>
                </div>
                  ...
                  <VRow v-for="(item, index) in encounterList" :key="index" class="d-flex align-center justify-space-between">   
                    <VCol cols="12">
                      <VRow>
                        ...
                        <VCol >
                          <VTextField v-model="encounterList[index]['ingredients']" label="Ingredients" variant="underlined" :rules="[requiredValidator]" density="default" style="width: 10rem;"></VTextField>                          
                        </VCol>                        
                        <VCol >                        
                          <VTextField v-model="encounterList[index]['quantity']" label="Quantity" variant="underlined" type="number" density="default" :rules="[requiredValidator]" style="width: 10rem;"></VTextField>                  
                        </VCol>
                        <VCol class="d-flex align-center">
                          <VTextField v-model="encounterList[index]['dosage']" label="Dosage" type="number" variant="underlined" density="default" :rules="[requiredValidator]" style="width: 5rem;"></VTextField>                  
                          <VSelect v-model="encounterList[index]['unit']" label="Unit" :items="units" :rules="[requiredValidator]" density="compact" variant="underlined" class="mt-3 ms-2" style="width: 1.5rem;" />                      
                      ...
                      <VRow class="d-flex mt-2">
                        <div style="min-width: 18rem;"></div>
                        <div class="w-75">
                          <VTable class="text-no-wrap border rounded mb-4">
                            <thead>
                              <tr>
                                  <th scope="col">Sub Item Name</th>
                                  <th scope="col">Sub Item Ingredients</th>
                                  <th scope="col">Dosage</th>
                              </tr>
                            </thead>
                            <tbody>
                              <tr v-for="(row, key) in item.subitems" :key="key">
                                <td>{{ row.sub_name }}</td>
                ...
                <VRow class="justify-end mt-4">
                  <VCol class="d-flex" cols="2">
                    <VBtn class="me-4" @click="addItems()"><VIcon color="white" icon="tabler-plus"/></VBtn>
                    <VBtn @click="removeItems()"><VIcon color="white" icon="tabler-trash"/></VBtn>
                  </VCol>
                </VRow>

                <VCol v-if="patient?.id && encounterList.length && !loading" class="mt-4">
                  <VBtn class="d-flex mx-auto" type="submit">Process Order <VIcon end icon="tabler-checkbox"/></VBtn>
                </VCol>
                <VCol v-if="loading" class="mt-4">
                  <VBtn class="d-flex mx-auto" size="large" style="width: 10rem;"><VProgressCircular color="white" indeterminate /></VBtn>
                </VCol>                
              </VForm>
            </VCardText>          
        </VCard>
          ...
          <VTable class="text-no-wrap">
            <thead>
              <tr>
                ...
                <th scope="col" class="font-weight-semibold text-center">Action</th>
              </tr>
            </thead>
            ...
            <tbody>
              ...
              >
                ...
                <td class="text-center">                  
                  <VBtn icon variant="text" color="default" size="x-small" @click="doCopyEncounter(item)">
                    <VIcon icon="tabler-copy" :size="22" color="primary"/>
                  </VBtn>
                  <VBtn icon variant="text" color="default" size="x-small" @click="isConfirmDialogVisible = true; removeEncounterId = item.id;">
