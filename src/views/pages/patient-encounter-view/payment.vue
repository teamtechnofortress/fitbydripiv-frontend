...
<template>
  <section>
            ...
            <VCardText class="my-4">
              <VRow class="mb-4">                        
                <VCol>
                  ...
                  />
                  <p
                    class="ms-1 mt-1 mb-0 text-warning"
                    style="font-size: 10pt;"
                  >
                    (* Valid Numbers: {{ patientIds() }} )
                  </p>
                </VCol>
              ...
              <VRow class="mt-4">
                <VCol
                  v-for="(row, index) in Object.keys(encounterList)"
                  :key="index"
                  cols="12"
                  class="border mb-4"
                >
                  ...
                  <VRow
                    v-for="(encounter, index) in encounterList[row]"
                    :key="index"
                  >    
                    <VCol cols="1">                                  
                      <VChip
                        v-if="encounter['is_add_on']"
                        label
                        color="warning"
                        class="mt-5"
                      >
                    ...
                    <VCol cols="3">
                      <VTextField
                        v-if="row == 'IV'"
                        :model-value="encounter['ingredients']"
                        label="Ingredients"
                        variant="underlined"
                        density="default"
                        readonly
                    ...
                    <VCol cols="1">
                      ...
                      <VTextField
                        v-else
                        :model-value="encounter['quantity']"
                        label="Quantity"
                        variant="underlined"
                        density="default"
                        readonly
                    ...
                    <VCol cols="1">
                      <VTextField
                        :model-value="encounter?.inventory?.price"
                        label="$ Price"
                        variant="underlined"
                        density="default"
                        type="number"
                    ...
                    <VCol cols="1">
                      <VTextField
                        :model-value="getRealValue((encounter?.inventory?.price || 0), encounter['discount'])"
                        label="NET"
                        variant="underlined"
                        density="default"
                        type="number"
                    ...
                    <VCol
                      cols="1"
                      class="text-center d-flex align-end justify-center"
                    >
                      <VBtn
                        icon
                        variant="text"
                        color="default"
                        size="x-small"
                        @click="isConfirmDialogVisible = true; rmRecordId = encounter['id'];"
                      >
                        <VIcon
                          icon="tabler-trash"
                          :size="22"
              ...
              <VRow class="mx-2 offset-10 text-right">
                <VCol
                  class="d-flex align-end offset-10"
                  cols="2"
                >
              ...
              <VRow class="mx-2 offset-10 text-right">                    
                ...
                >
                  ...
                  <VTextField
                    :model-value="Math.ceil(getSubTotalPrice()*tax)/100"
                    label="USD"
                    variant="underlined"
                    density="default"
                    type="number"
        ...
        <VRow class="mt-4">
          ...
          <VCol cols="12">
            <VTable class="text-no-wrap border rounded mb-4">
              ...
              <tbody>
                <tr
                  v-for="(invoice, index) in patient?.invoice"
                  :key="index"
                >
                  <td>{{ index+1 }}</td>
                  <td>                    
                    <table class="border w-100 pa-4">
                      ...
                      <tbody>
                        <tr
                          v-for="(item, index) in [].concat(...(Object.values(JSON.parse(invoice.data))))"
                          :key="index"
                        >
                          <td class="text-left">
                            {{ item.type }}
    ...
    <PaymentConfirmDialog
      ...
      v-model:isConfirmed="isConfirmed"
    />
  </section>
</template>