...
import CustomerServiceDialog from "./customer-service-dialog.vue";
...
const getAllCustomerReport = () => {
  Network.getRequest(Const.ALL_CUSTOMER_SERVICE_REPORT, {}, {}, (response)=>{
...
function doConfirm(value){
  if(value){
    Network.getRequest(`${Const.DELETE_CUSTOMER_SERVICE_REPORT}/${rmRecordId.value}`, {}, {}, 
      (response) => {
        if(response.data.success){
          toast.success("Successfully Deleted Customer Service Report.");              
...
<template>
  <section>
          ...
          <VTable class="text-no-wrap">
            <thead>
              <tr>
                ...
                <th scope="col" class="font-weight-semibold">Reward Sales</th>
                <th scope="col" class="font-weight-semibold">Add On</th>                
            ...
            <tbody>
              ...
              >
                ...
                <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.arrive_stime?.slice(0, 5) }} ~ {{ item.arrive_etime?.slice(0, 5) }}
                  </div>
                </td>                          
                <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.reward_sales ? 'Yes' : 'No' }}
                  </div>
                </td> 
                <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.add_on_beg ? 'Yes' : 'No' }}
                  </div>
                </td>                 
                <td class="text-center">
                  <div class="v-avatar-group">
    ...
    <CustomerServiceDialog v-model="showChartHistoryDialog" @refresh="getAllCustomerReport()"/>
  </section>  
</template>
