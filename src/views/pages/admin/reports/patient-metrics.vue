...
import PatientMetricsDialog from "./patient-metrics-dialog.vue";

const showChartHistoryDialog = ref(false);
const toast = useToast();

const getAllPatientMetrics = () => {
  Network.getRequest(Const.ALL_PATIENT_METRICS, {}, {}, (response)=>{
...
function doConfirm(value){
  if(value){
    Network.getRequest(`${Const.DELETE_PATIENT_METRICS}/${rmRecordId.value}`, {}, {}, 
      (response) => {
        if(response.data.success){
          toast.success("Successfully Deleted Patient Metrics.");              
...
const customerServiceReportList = computed(() => {
...
<template>
  <section>
          ...
          <VTable class="text-no-wrap">
            <thead>
              <tr>
                ...
                <th scope="col" class="font-weight-semibold">Add On Purchase</th>
                <th scope="col" class="font-weight-semibold">Reward Usage</th>
                <th scope="col" class="font-weight-semibold">Utilized Discount</th>
            ...
            <tbody>
              <tr
                v-for="(item, index) in customerServiceReportList"
                :key="index"
                style="height: 3.5rem;"          
              >
                ...
                <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.add_on_purchase ? 'Yes' : 'No' }}
                  </div>
                </td> 
                <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.reward_usage ? 'Yes' : 'No' }}
                  </div>
                </td> 
                <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.utilized_discount ? 'Yes' : 'No' }}
                  </div>
                </td> 
                <td class="text-center">
                  <div class="v-avatar-group">
    ...
    <PatientMetricsDialog v-model="showChartHistoryDialog" @refresh="getAllPatientMetrics()"/>
  </section>  
</template>
