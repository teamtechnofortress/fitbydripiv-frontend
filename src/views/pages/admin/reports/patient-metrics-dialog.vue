...
const getRangeDate = (type) => {
  if(type == 'daily'){
    ...
    return {"range_sdate": patientMetrics.value.range_sdate, "range_edate": patientMetrics.value.range_edate};
  }
}
...
function saveForm(){ 
  refVForm.value?.validate().then(({ valid: isValid }) => {
    loading.value = true;
    if (isValid){       
      const rangeDate = getRangeDate(patientMetrics.value.frequency);
      Network.postRequest(Const.SAVE_PATIENT_METRICS, {}, {...patientMetrics.value, ...rangeDate}, (response)=>{        
        loading.value = false;
...
onMounted(()=>{
  patientMetrics.value.range_sdate = ref(new Date(new Date().setDate(new Date().getDate() - 10)).toISOString().slice(0, 10));
  patientMetrics.value.range_edate = ref(new Date().toJSON().slice(0, 10));  
});
</script>
    ...
    <VCard>
      ...
      <VCardText>
        <VRow>
          <VRadioGroup
              class="col-12"
              v-model="patientMetrics['frequency']"
              inline   
        ...
        <VRow>
          ...
          >
            ...
            <input type="date" v-model="patientMetrics['range_sdate']"/>
          </VCol> 
          <VCol
            cols="6"
            class="d-flex align-center"
          >
            ...
            <input type="date" v-model="patientMetrics['range_edate']"/>            
          </VCol> 
        </VRow>

        <VDivider class="mxt-4 pt-4"/>
        <VRow>
          <VRadioGroup
              class="col-12"
              v-model="patientMetrics['type']"
              inline   
              :rules="[requiredValidator]"           
            >
            <VCol cols="4">
        ...
        <VRow>
          <VCol 
            ...
            >            
            <VCheckbox
              v-model="patientMetrics['add_on_purchase']"
          ...
          <VCol 
            ...
            >            
            <VCheckbox
              v-model="patientMetrics['reward_usage']"
              label="REWARD USAGE"
            />
          </VCol>          
          <VCol cols="4">
            <VCheckbox
              v-model="patientMetrics['utilized_discount']"
              label="UTILIZED DISCOUNT"
            />
          </VCol>          
        </VRow>        
        ...
        <VRow class="mt-2">
          <VCol cols="4" class="d-flex align-center">
            <VTextField v-model="patientMetrics['email']" type="email" label="EMAIL" :rules="[requiredValidator, emailValidator]"></VTextField>
          </VCol> 
        </VRow>
