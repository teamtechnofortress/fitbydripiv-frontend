...
const getRangeDate = (type) => {
  if(type == 'daily'){
    ...
    return {"range_sdate": payroll.value.range_sdate, "range_edate": payroll.value.range_edate};
  }
}
...
function saveForm(){ 
  refVForm.value?.validate().then(({ valid: isValid }) => {
    loading.value = true;
    if (isValid){   
      const rangeDate = getRangeDate(payroll.value.frequency);
      Network.postRequest(Const.SAVE_PAYROLL_REPORT, {}, {...payroll.value, ...rangeDate}, (response)=>{        
        loading.value = false;
...
onMounted(()=>{
  payroll.value.range_sdate = ref(new Date(new Date().setDate(new Date().getDate() - 10)).toISOString().slice(0, 10));
  payroll.value.range_edate = ref(new Date().toJSON().slice(0, 10));  
});
</script>
      ...
      <VCardText>
        ...
        <VRow>
          ...
          >
            ...
            <input type="date" v-model="payroll['range_sdate']"/>
          </VCol> 
          <VCol
            cols="6"
            class="d-flex align-center"
          >
            ...
            <input type="date" v-model="payroll['range_edate']"/>            
          </VCol> 
        </VRow>
        ...
        <VRow>
          <VCol cols="4">
            <VCheckbox
              label="HOURS WORKED"
              v-model="payroll['hours_worked']"
            />
          </VCol>
          <VCol cols="4">
            <VCheckbox
              label="SALARY"
              v-model="payroll['salary']"
            />
          </VCol>
          <VCol cols="4">
            <VCheckbox
              label="CALCULATED OVERTIME"
              v-model="payroll['calculated_overtime']"
            />
          </VCol>
        </VRow>
        ...
        <VRow class="mt-2">
          <VCol cols="4" class="d-flex align-center">
            <VTextField v-model="payroll['email']" type="email" label="EMAIL" :rules="[requiredValidator, emailValidator]"></VTextField>
          </VCol> 
        </VRow>
