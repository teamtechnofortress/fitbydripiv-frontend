...
import InvoicingSalesReportDialog from "./invoicing-sales-report-dialog.vue";

const showDialog = ref(false);
const toast = useToast();

const getAllInvoicingSalesReports = () => {
  Network.getRequest(Const.ALL_INVOICING_SALES_REPORT, {}, {}, (response)=>{
...
function doConfirm(value){
  if(value){
    Network.getRequest(`${Const.DELETE_INVOICING_SALES_REPORT}/${rmRecordId.value}`, {}, {}, 
      (response) => {
        if(response.data.success){
          toast.success("Successfully Deleted Email/Text Reward Report.");              
...
<template>
  <section>
      ...
      <VCol>        
        <VCard class="px-1">
          <VRow class="my-4 mx-1">
            <VBtn color="primary" @click="showDialog = true">
          ...
          <VTable class="text-no-wrap">
            <thead>
              <tr>
                ...
                <th scope="col" class="font-weight-semibold">CreditCard</th>
                <th scope="col" class="font-weight-semibold">Cash</th>
                <th scope="col" class="font-weight-semibold">Transfer</th>
                <th scope="col" class="font-weight-semibold">Paypal</th>
                <th scope="col" class="font-weight-semibold">Venmo</th>                
                <th scope="col" class="font-weight-semibold">CashApp</th>                
                <th scope="col" class="font-weight-semibold">Crypto</th>
                <th scope="col" class="font-weight-semibold">Sales Totals</th>
                <th scope="col" class="font-weight-semibold">Profit</th>
                <th scope="col" class="font-weight-semibold">Margin</th>
                <th scope="col" class="font-weight-semibold">Sales tax collected</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.sales_totals ? 'Yes' : 'No' }}
                  </div>
                </td>                 
                <!-- <td class="text-center">
                  <div class="v-avatar-group">
                    {{ item.sales_detail ? 'Yes' : 'No' }}
                  </div>
                </td> -->
                <td class="text-center">
                  <div class="v-avatar-group">
            ...
            <tfoot v-show="!historyList.length">
              <tr>
                <td colspan="17" class="text-center text-body-1">No data available</td>
              </tr>
            </tfoot>
          </VTable>
          <!-- !SECTION -->   
    ...
    <InvoicingSalesReportDialog v-model="showDialog" @refresh="getAllInvoicingSalesReports()"/>
  </section>  
</template>
