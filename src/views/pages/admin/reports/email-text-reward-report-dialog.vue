...
const getRangeDate = (type) => {
  if(type == 'daily'){
    ...
    return {"range_sdate": reward.value.range_sdate, "range_edate": reward.value.range_edate};
  }
}
...
function saveForm(){ 
  refVForm.value?.validate().then(({ valid: isValid }) => {
    loading.value = true;
    if (isValid){       
      const rangeDate = getRangeDate(reward.value.frequency);     
      Network.postRequest(Const.SAVE_EMAIL_TEXT_REWARD_REPORT, {}, {...reward.value, ...rangeDate}, (response)=>{        
        loading.value = false;
...
onMounted(()=>{
  reward.value.range_sdate = ref(new Date(new Date().setDate(new Date().getDate() - 10)).toISOString().slice(0, 10));
  reward.value.range_edate = ref(new Date().toJSON().slice(0, 10));  
});
</script>
      ...
      <VCardText>
        ...
        <VRow>
          ...
          >
            ...
            <input type="date" v-model="reward['range_sdate']"/>
          </VCol> 
          <VCol
            cols="6"
            class="d-flex align-center"
          >
            ...
            <input type="date" v-model="reward['range_edate']"/>            
          </VCol> 
        </VRow>
        ...
        <VRow>
          <VCol 
            ...
            >            
            <VCheckbox
              v-model="reward['email_sent']"
              label="EMAIL MARKETING SENT"
            />
          </VCol> 
          <VCol 
            cols="6"
            ...
            >            
            <VCheckbox
              v-model="reward['text_sent']"
              label="TEXT MESSAGES MARKETING SENT"
            />
          </VCol>                 
        </VRow>
        ...
        <VRow>          
          <VCol 
            ...
            >            
            <VCheckbox
              v-model="reward['reward_sent']"
              label="REWARDS MARKETING SENT"
            />
          </VCol>
          <VCol 
            cols="6"
            ...
            >            
            <VCheckbox
              v-model="reward['birth_sent']"
              label="BIRTHDAY MARKETING SENT"
            />
          </VCol> 
        </VRow>        
        ...
        <VRow class="mt-2">
          <VCol cols="4" class="d-flex align-center">
            <VTextField v-model="reward['email']" type="email" label="EMAIL" :rules="[requiredValidator, emailValidator]"></VTextField>
          </VCol> 
        </VRow>
