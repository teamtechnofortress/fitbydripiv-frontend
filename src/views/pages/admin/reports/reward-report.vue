...
import RewardReportDialog from "./reward-report-dialog.vue";

const showChartHistoryDialog = ref(false);
const toast = useToast();

const getAllRewardReports = () => {
  Network.getRequest(Const.ALL_REWARD_REPORT, {}, {}, (response)=>{
...
function doConfirm(value){
  if(value){
    Network.getRequest(`${Const.DELETE_REWARD_REPORT}/${rmRecordId.value}`, {}, {}, 
      (response) => {
        if(response.data.success){
          toast.success("Successfully Deleted Reward Report.");              
          getAllRewardReports();
        }else{
...
<template>
  <section>
    ...
    <RewardReportDialog v-model="showChartHistoryDialog" @refresh="getAllRewardReports()"/>
  </section>  
</template>
