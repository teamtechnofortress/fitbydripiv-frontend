...
const getRangeDate = (type) => {
  if(type == 'daily'){
    ...
    return {"range_sdate": productMetrics.value.range_sdate, "range_edate": productMetrics.value.range_edate};
  }
}
...
function saveForm(){ 
  refVForm.value?.validate().then(({ valid: isValid }) => {
    loading.value = true;
    if (isValid){     
      const rangeDate = getRangeDate(productMetrics.value.frequency);  
      Network.postRequest(Const.SAVE_PRODUCT_METRICS, {}, {...productMetrics.value, ...rangeDate}, (response)=>{        
        loading.value = false;
...
onMounted(()=>{
  productMetrics.value.range_sdate = ref(new Date(new Date().setDate(new Date().getDate() - 10)).toISOString().slice(0, 10));
  productMetrics.value.range_edate = ref(new Date().toJSON().slice(0, 10));  
});
</script>
    ...
    <VCard>
      ...
      <VCardText>
        <VRow>
          <VRadioGroup
              class="col-12"
              v-model="productMetrics['frequency']"
              inline   
        ...
        <VRow>
          ...
          >
            ...
            <input type="date" v-model="productMetrics['range_sdate']"/>
          </VCol> 
          <VCol
            cols="6"
            class="d-flex align-center"
          >
            ...
            <input type="date" v-model="productMetrics['range_edate']"/>            
          </VCol> 
        </VRow>
        ...
        <VRow>
          <VCol 
            ...
            >            
            <VCheckbox
              v-model="productMetrics['isSemaglutide']"
              label="SEMAGLUTIDE"
            />
          </VCol>          
          <VCol 
            ...
            >            
            <VCheckbox
              v-model="productMetrics['isTirzepatide']"
              label="TIRZEPATIDE"
            />
          </VCol>          
        </VRow>
        ...
        <VRow>
          <VCol cols="3">
            <VCheckbox
              v-model="productMetrics['isIV']"
              label="IV"              
            />
          </VCol>
          <VCol cols="3">
            <VCheckbox
              v-model="productMetrics['isInjections']"    
              label="INJECTIONS" 
            />
          </VCol>
          <VCol cols="3">
            <VCheckbox
              v-model="productMetrics['isPeptides']"
              label="PEPTIDES"
            />
          </VCol>  
          <VCol cols="3">            
            <VCheckbox
              v-model="productMetrics['isOther']"
              label="OTHER"
            />
          </VCol>           
        </VRow>
        ...
        <VRow class="mt-2">
          <VCol cols="4" class="d-flex align-center">
            <VTextField v-model="productMetrics['email']" type="email" label="EMAIL" :rules="[requiredValidator, emailValidator]"></VTextField>
          </VCol> 
        </VRow>
