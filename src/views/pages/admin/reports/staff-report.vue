...
import StaffReportDialog from "./staff-report-dialog.vue";

const showChartHistoryDialog = ref(false);
const toast = useToast();

const getAllStaffReports = () => {
  Network.getRequest(Const.ALL_STAFF_REPORT, {}, {}, (response)=>{
...
function doConfirm(value){
  if(value){
    Network.getRequest(`${Const.DELETE_STAFF_REPORT}/${rmRecordId.value}`, {}, {}, 
      (response) => {
        if(response.data.success){
          toast.success("Successfully Deleted Staff Report.");              
          getAllStaffReports();
        }else{
...
<template>
  <section>
    ...
    <StaffReportDialog v-model="showChartHistoryDialog" @refresh="getAllStaffReports()"/>
  </section>  
</template>
