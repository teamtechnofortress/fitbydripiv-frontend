...
function saveForm(){  
    refVForm.value?.validate().then(({ valid: isValid }) => {    
        if (isValid){
            ...
            Network.postRequest(Const.ADD_PROCEDURE_PLAN_NOTES, {}, {...complaint.value, notes: complaintText.value}, 
                (response) => {
                    isLoading.value = false;
                    if(response.data.success){
                        toast.success("Procedure Plan Notes saved successfully!");   
                        getNotes();                     
                    }else{
...
function getNotes(){
    isLoading.value = true;
    Network.getRequest(Const.GET_PROCEDURE_PLAN_NOTES, {}, {}, 
        (response) => {
            isLoading.value = false;
            if(response.data.success){                
            ...
            <VCard>
                <VCardText class="mb-4">
                    ...
                    <VRow class="mt-4 pt-4">
                        <VBtn v-if="!isLoading" class="d-flex mx-auto" color="primary" type="submit">UDPATE NOTES</VBtn>
