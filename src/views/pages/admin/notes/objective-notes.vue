...
const physicalExamText = ref('');

const evaluationText = ref('');
const ivEvaluation = ref("");
const wlEvaluation = ref("");
const ptEvaluation = ref("");

const vitalSignText = ref('');
const ivVitalSign = ref("");
const wlVitalSign = ref("");
const ptVitalSign = ref("");
...
function saveForm(){  
    refVForm.value?.validate().then(({ valid: isValid }) => {    
        if (isValid){
            ...
            Network.postRequest(Const.ADD_ADMIN_OBJECT_NOTES, {}, {
                ...complaint.value, physical_exam: physicalExamText.value, evaluation: evaluationText.value, vital_sign: vitalSignText.value
            }, 
                (response) => {
                    isLoading.value = false;
                    if(response.data.success){
                        toast.success("Objective Notes saved successfully!");   
                        getNotes();                     
                    }else{
    ...
    Network.getRequest(Const.GET_ADMIN_NOTES, {}, {}, 
        (response) => {
            ...
            if(response.data.success){                
                ...
                if(notesObj){
                    //physical_exam
                    ivContent.value         = notesObj.find(item => item.treatment_type == 'IV Therapy')?.physical_exam || ivContent.value;
                    weightLossContent.value = notesObj.find(item => item.treatment_type == 'Weight Loss')?.physical_exam || weightLossContent.value;
                    peptideContent.value    = notesObj.find(item => item.treatment_type == 'Injectables')?.physical_exam || peptideContent.value;
                    
                    //evaluation                    
                    ivEvaluation.value   = notesObj.find(item => item.treatment_type == 'IV Therapy')?.evaluation || ivContent.value;
                    wlEvaluation.value   = notesObj.find(item => item.treatment_type == 'Weight Loss')?.evaluation || weightLossContent.value;
                    ptEvaluation.value   = notesObj.find(item => item.treatment_type == 'Injectables')?.evaluation || peptideContent.value;

                    //vital_sign
                    ivVitalSign.value   = notesObj.find(item => item.treatment_type == 'IV Therapy')?.vital_sign || ivContent.value;
                    wlVitalSign.value   = notesObj.find(item => item.treatment_type == 'Weight Loss')?.vital_sign || weightLossContent.value;
                    ptVitalSign.value   = notesObj.find(item => item.treatment_type == 'Injectables')?.vital_sign || peptideContent.value;
                }
            }else{
                console.log(`Error: ${response.data.err_msg}`);
            }
...
function updateType(_type){
    if(_type == 'IV Therapy'){
        physicalExamText.value = ivContent.value;
        evaluationText.value = ivEvaluation.value;
        vitalSignText.value = ivVitalSign.value;
    }else if(_type == 'Weight Loss'){
        physicalExamText.value = weightLossContent.value;
        evaluationText.value = wlEvaluation.value;
        vitalSignText.value = wlVitalSign.value;
    }else{
        physicalExamText.value = peptideContent.value;
        evaluationText.value = ptEvaluation.value;
        vitalSignText.value = ptVitalSign.value;
    }
}
</script>
            ...
            <VCard>
                <VCardText class="mb-4">
                    ...
                    <VRow class="mt-2">
                        <VCol cols="12" style="min-height: 150px;">
                            <div class="row d-flex align-center">                                    
                                <VTextarea v-model="physicalExamText" rows="10" :rules="[requiredValidator]"></VTextarea>
                            </div>                            
                        </VCol>                        
                    </VRow>   
                    <VRow class="mb-4">
                    ...
                    <VRow>
                        <VCol cols="12" style="min-height: 150px;">
                            <div class="row d-flex align-center">                                    
                                <VTextarea v-model="evaluationText" rows="10" :rules="[requiredValidator]"></VTextarea>
                            </div>                            
                        </VCol>                        
                    </VRow>
                    <VRow class="mb-4">
                    ...
                    <VRow>
                        <VCol cols="12" style="min-height: 150px;">
                            <div class="row d-flex align-center">                                    
                                <VTextarea v-model="vitalSignText" rows="10" :rules="[requiredValidator]"></VTextarea>
                            </div>                            
                        </VCol>                        
                    </VRow>
                    <VRow class="mb-4">
                    ...
                    <VRow class="mt-4 pt-4">
                        <VBtn v-if="!isLoading" class="d-flex mx-auto" color="primary" type="submit">UDPATE OBJECTIVE NOTES</VBtn>
