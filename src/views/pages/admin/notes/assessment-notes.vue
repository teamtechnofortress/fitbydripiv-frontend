...
function saveForm(){  
    refVForm.value?.validate().then(({ valid: isValid }) => {    
        if (isValid){
            ...
            Network.postRequest(Const.ADD_ADMIN_ASSESSMENT_NOTES, {}, { ...complaint.value, assessment: physicalExamText.value }, 
                (response) => {
                    isLoading.value = false;
                    if(response.data.success){
                        toast.success("Assessment Notes saved successfully!");   
                        getNotes();                     
                    }else{
    ...
    Network.getRequest(Const.GET_ADMIN_NOTES, {}, {}, 
        (response) => {
            ...
            if(response.data.success){                
                ...
                if(notesObj){
                    //assessment
                    ivContent.value         = notesObj.find(item => item.treatment_type == 'IV Therapy')?.assessment || ivContent.value;
                    weightLossContent.value = notesObj.find(item => item.treatment_type == 'Weight Loss')?.assessment || weightLossContent.value;
                    peptideContent.value    = notesObj.find(item => item.treatment_type == 'Injectables')?.assessment || peptideContent.value;
                }
            }else{
                console.log(`Error: ${response.data.err_msg}`);
            }
            ...
            <VCard>
                <VCardText class="mb-4">
                    ...
                    <VRow class="mt-4 pt-4">
                        <VBtn v-if="!isLoading" class="d-flex mx-auto" color="primary" type="submit">UDPATE ASSESSMENT NOTES</VBtn>
