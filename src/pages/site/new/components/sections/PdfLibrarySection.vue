...
const documentItem = computed(() => {
  ...
  const downloadUrl = raw.download_url || raw.pdf_url || raw.item?.download_url || null
  ...
  <section class="py-12 px-4 border-t border-gray-200 bg-white">
    <div class="max-w-6xl mx-auto">
      <div class="text-center mb-8">
        ...
        <p v-if="uploadHelp" class="text-xs text-gray-500 mt-2">{{ uploadHelp }}</p>
      </div>

      <div v-if="!documentItem" class="card p-6 text-center text-sm text-gray-600">
      ...
      <template v-else>
        <div class="card overflow-hidden">
          <div class="px-5 py-4 border-b border-gray-200 bg-white flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="min-w-0">
              <h3 class="text-base font-semibold text-gray-900 truncate">{{ documentItem.label }}</h3>
              <p class="text-xs text-gray-600 mt-0.5">PDF Viewer</p>
            </div>
            ...
            >
              <div class="p-6 text-sm text-slate-600">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  class="text-blue-600 hover:text-blue-700 ml-1 font-semibold"
                >
          ...
          >
            <div>
              <p class="text-sm font-semibold text-gray-800">Preview not available</p>
