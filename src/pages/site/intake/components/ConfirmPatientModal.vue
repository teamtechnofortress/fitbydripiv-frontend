...
.modal-overlay {
  ...
  --radius:      12px;
  --radius-lg:   18px;
  --shadow-lg:   0 24px 48px rgba(15,23,42,.15), 0 4px 10px rgba(15,23,42,.08);
...
.modal-overlay {
  ...
  z-index: 999;
  padding: 20px;
  animation: overlay-in .28s ease both;
}
...
.modal-content {
  ...
  width: 100%;
  overflow: hidden;
  animation: modal-in .4s cubic-bezier(.34,1.46,.64,1) both;
}
...
.detail-icon {
  width: 34px;
  height: 34px;
  background: linear-gradient(135deg, var(--green-light) 0%, rgba(37,99,235,.07) 100%);
  border: 1px solid var(--green-mid);
  border-radius: 8px;
  display: flex;
  align-items: center;
...
/* ── Transition ────────────────────────────────────── */
.modal-fade-enter-active { transition: opacity .28s ease; }
